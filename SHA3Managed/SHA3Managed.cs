...
namespace SHA3Managed
{
    public class SHA3Managed
    {
        ...
        public byte[] Hash(byte[] data, int inlen)
        {
            ...
            {
                ulong[] utemps = new ulong[sizeInBytes];
                Buffer.BlockCopy(data, inb, utemps, 0, sizeInBytes);
                KeccakF(utemps, sizeInBytes / 8);
            }
            byte[] temp = new byte[sizeInBytes];
            Buffer.BlockCopy(data, inb, temp, 0, inlen);

            //    padding
            temp[inlen++] = 1;
            temp[sizeInBytes - 1] |= 0x80;

            ulong[] utemps2 = new ulong[sizeInBytes / 8];
            Buffer.BlockCopy(temp, 0, utemps2, 0, sizeInBytes);
            KeccakF(utemps2, sizeInBytes / 8);
